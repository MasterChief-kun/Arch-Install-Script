...
read wipe
echo Unallocated Space: $(parted $diskname unit GB print free | grep 'Free Space' | tail -n1 | awk '{print $3}')  
echo " "
...
read swappart
echo Unallocated Space: $(parted $diskname unit GB print free | grep 'Free Space' | tail -n1 | awk '{print $3}')
echo Please verify the information you have entered, as after this irreversible changes will be made:
